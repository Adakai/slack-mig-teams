...
def import_message(
    ...
    *,
    ...
    # NOTE: user attribution (“from”) can vary per-tenant policy. Start system-attributed; add later if needed.
    ...
    tok = _get_token()
    url = f"{GRAPH}/teams/{team_id}/channels/{channel_id}/messages/import"
    r = requests.post(url, headers=_headers(tok), data=json.dumps(payload), timeout=60)
    if r.status_code not in (201, 202):
        raise RuntimeError(f"Import message failed: {r.status_code} {r.text[:500]}")
...
def complete_team_migration(team_id: str, dry_run=False):
    ...
    tok = _get_token()
    r = requests.post(
        f"{GRAPH}/teams/{team_id}/completeMigration", headers=_headers(tok), timeout=60
    )
    if r.status_code not in (200, 202, 204):
        raise RuntimeError(
